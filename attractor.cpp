 ...
 *
 */
Attractor::Attractor(int num, int p1, int p2, int p3, int p4, int p5, int p6, int p7, int p8)
{
    numpts = num;
    colormode = 0;
...
void Attractor::generatePoints()
{
    ...
    point* next;
    int nextcube;
    //Iterate a few times to get a point near the cubeiterator
    for(int i = 0; i < 20; i++){
        nextcube = rand() % numcubes;
        next = applytransform( ic, transforms.at(nextcube) );
        ic->x = next->x;
        ic->y = next->y;
        ic->z = next->z;
    }

    ic->cube = *cubecolor(usedcubes.at(nextcube));
    ic->past = *cubecolor(usedcubes.at(nextcube));

    //Start the real iteration
    pointcloud.push_back(ic);

    for(int i = 0; i < numpts-1; i++){
        nextcube = rand() % numcubes;
        next = applytransform( pointcloud[i], transforms.at(nextcube) );
        next->cube = *cubecolor(usedcubes.at(nextcube));
        next->past = *averagecolor( &next->cube, &pointcloud[i]->past );
        pointcloud.push_back(next);
    }
}
...
void Attractor::display()
{
    ...
    if(colormode == 2){
        for(int i = 0; i < numpts; i++)
        {
            useColor(Color((pointcloud[i]->x+1)*.5,(pointcloud[i]->y+1)*.5,(pointcloud[i]->z+1)*.5));
            glVertex3f(pointcloud[i]->x,pointcloud[i]->y,pointcloud[i]->z);
        }
    }
 ...
 * ***********
 *      Fills in transforms, the IFS affine contratction maps which describe the cubeiterator
 *      Also calculates numcubes
 */
void Attractor::fillTransformQueue()
{
    transforms.clear();
    usedcubes.clear();
 ...
 * Change Parameters
 * ***********
 *     Sets the parameters of the cubeiterator
 */
void Attractor::changeParameters(int p1, int p2, int p3, int p4, int p5, int p6, int p7, int p8)
{
    params[0] = p1;
    params[1] = p2;
    params[2] = p3;
 ...
 *     Sets the parameters of the cubeiterator
 */
void Attractor::setNumpts(int num)
{
    numpts = num;
 ...
 *     Sets the colormode
 */
void Attractor::setColormode(int mode)
{
    colormode = mode;
 ...
 */
color* Attractor::cubecolor(int cube)
{
    color* c = new color;
 ...
 */
color* Attractor::averagecolor(color *c1, color *c2){
...
point* Attractor::applytransform(point* p, affine* T){
 ...
 *
 */
affine* Attractor::symmetrytransform(int sym, int cube){
